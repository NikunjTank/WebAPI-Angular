...
{
    public class SpellingAutoCorrect
    {
        private Dictionary<String, int> _dictionary = new Dictionary<String, int>();
        private static Regex _wordRegex = new Regex("[a-z]+", RegexOptions.Compiled);

        public SpellingAutoCorrect()
        {
            var spellingAutoCorrectFile = HttpContext.Current.Server.MapPath(ConfigurationManager.AppSettings["SpellingAutoCorrectFile"].ToString());

            string fileContent = File.ReadAllText(spellingAutoCorrectFile);
            ...
            foreach (var item in wordList)
            {
                ...
                {
                    string trimmedWord = Regex.Replace(word, @"[^0-9a-zA-Z]+", "").Trim().ToLower();
        ...
        public string Correct(string word)
        {
            ...
            else
            {
                ...
                {
                    if (word == item.Key.Substring(0, (item.Key.Length >= word.Length) ? word.Length : item.Key.Length))
                        return word;
                }
            }
