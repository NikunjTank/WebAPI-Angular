...
export class UserProfileUserService {
    ...
    getUserProfile(getUserProfileRequest: GetUserProfileRequest): Observable<any> {
        ...
        return this._http.get(Global.API_SITE + "user/api/userprofiles", requestOptions)
    ...
    updateProfile(model: any): Observable<any> {
        ...
        return this._http.post(Global.API_SITE + "user/api/userprofiles/update", body, options)
            .pipe(map((response: Response) => <any>response.json()), catchError(e => throwError(e)));
    }
    ...
    sendOTPForUserProfile(model: any): Observable<any> {
        ...
        let headers = new Headers({ 'Content-Type': 'application/json', 'Authorization': this._global.getUserToken() });
        let options = new RequestOptions({ headers: headers });
        return this._http.post(Global.API_SITE + "user/api/sendotpforuserprofile", body, options)
            .pipe(map((response: Response) => <any>response.json()), catchError(e => throwError(e)));
    }
}